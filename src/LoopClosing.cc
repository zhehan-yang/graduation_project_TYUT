...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())		//如果说有新的关键帧在队列中等待
        {
            // Detect loop candidates and check covisibility consistency
            if(DetectLoop())		//如果侦测到回环
            {
               // Compute similarity transformation [sR|t]
               // In the stereo/RGBD case s=1
               if(ComputeSim3())	//sim3计算
               {
...
//从队列中取出一个关键帧，作为当前闭环检测的关键帧
    ...
    //如果说距离上一次闭环还没多久10个帧或者map中的关键帧还没有10个帧，那么布景性闭环检测
    ...
    //如果认为当前关键帧具有回环关系的关键帧，这两帧的相似度不应该低于mincore
...
bool LoopClosing::ComputeSim3()
{
    ...
    // global match score
    cv::Mat database = cv::Mat::ones(4096, mvpEnoughConsistentCandidates.size(),CV_32F);
    for(size_t i=0; i<mvpEnoughConsistentCandidates.size(); i++)
    {
        mvpEnoughConsistentCandidates[i]->_globaldescriptors.copyTo(database.col(i));

    }

    cv::Mat a = mpCurrentKF->_globaldescriptors.t();
    cv::Mat iii = cv::Mat::ones(1, mvpEnoughConsistentCandidates.size(),CV_32F);
    cv::Mat goal = 2*(iii - a* database);
    double value = 100000;
    int Simiindex=0;

    if(goal.cols>0) {
        for (size_t i = 0; i < goal.cols; i++) {
            if (value > goal.at<double>(0, i)) {
                Simiindex = i;
                value = goal.at<double>(0, i);
            }
        }
    }
