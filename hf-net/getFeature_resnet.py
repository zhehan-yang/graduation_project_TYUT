...
def infer(imagePath, model):
    print(imagePath, model)
    image = cv2.imread(imagePath)
    begintime = time.time()
    output = model.inference(image)
    endtime = time.time()
    print(type(endtime - begintime), endtime - begintime)

    # return output
    # return output['local_descriptors'], output['global_descriptor'], output['keypoints'],output['local_descriptors'][0]
    dem = output['local_descriptors'].shape[0]
    print(f"type(dem):{type(dem)}")
    ...
    # # localDes = np.asarray(output['local_descriptors'], dtype=c_float)
    # # globalDes = np.asarray(output['global_descriptor'], dtype=c_float)
    # # localIndex = np.asarray(output['keypoints'], dtype=c_float)
    # if not localDes.flags["C_CONTIGUOUS"]:
    #     localDes = np.ascontiguous(localDes, dtype=localDes.dtype)
    #     print("localDes isn't continuous")
    # if not globalDes.flags["C_CONTIGUOUS"]:
    #     globalDes = np.ascontiguous(globalDes, dtype=globalDes.dtype)
    #     print("globalDes isn't continuous")
    # if not localIndex.flags["C_CONTIGUOUS"]:
    #     localIndex = np.ascontiguous(localIndex, dtype=localIndex.dtype)
    #     print("localIndex isn't continuous")
    # localDesPtr = cast(localDes.ctypes.data, POINTER(c_float))
    # globalDesPtr = cast(globalDes.ctypes.data, POINTER(c_float))
    # localIndexPtr = cast(localIndex.ctypes.data, POINTER(c_float))
    # return dem, localDesPtr, globalDesPtr, localIndexPtr
    # return dem
